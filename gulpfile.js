...
var sassFrontendSource        = [ 'dev/scss/maxfootedb-main-frontend.scss' ];
var sassBackendSource         = [ 'dev/scss/maxfootedb-main-admin.scss' ];
...
gulp.task( 'concatAdminJs', function() {
	return gulp.src(jsBackendSource ) // use jsSources
		.pipe(concat( 'maxfootedb_admin.min.js' ) ) // Concat to a file named 'script.js'
...
gulp.task( 'concatFrontendJs', function() {
	return gulp.src(jsFrontendSource ) // use jsSources
		.pipe(concat( 'maxfootedb_frontend.min.js' ) ) // Concat to a file named 'script.js'
		.pipe(uglify() ) // Uglify concatenated file
		.pipe(gulp.dest( 'assets/js' ) ); // The destination for the concatenated and uglified file
});

gulp.task( 'copyassets', function () {
	return gulp.src([ './assets/**/*' ], {base: './'}).pipe(gulp.dest( '../maxfootedb_dist/Maxfootedb-Distribution' ) );
});

gulp.task( 'copyincludes', function () {
	return gulp.src([ './includes/**/*' ], {base: './'}).pipe(gulp.dest( '../maxfootedb_dist/Maxfootedb-Distribution' ) );
});

gulp.task( 'copyreadme', function () {
	return gulp.src([ './readme.txt' ], {base: './'}).pipe(gulp.dest( '../maxfootedb_dist/Maxfootedb-Distribution' ) );
});

gulp.task( 'copylang', function () {
	return gulp.src([ './languages/**/*' ], {base: './'}).pipe(gulp.dest( '../maxfootedb_dist/Maxfootedb-Distribution' ) );
});

gulp.task( 'copymainfile', function () {
	return gulp.src([ './maxfootedb.php' ], {base: './'}).pipe(gulp.dest( '../maxfootedb_dist/Maxfootedb-Distribution' ) );
});

gulp.task( 'zip', function () {
	return gulp.src( '../maxfootedb_dist/Maxfootedb-Distribution/**' )
		.pipe(zip( 'maxfootedb.zip' ) )
		.pipe(gulp.dest( '../maxfootedb_dist/Maxfootedb-Distribution' ) );
});

gulp.task( 'cleanzip', function(cb) {
	return del([ '../maxfootedb_dist/Maxfootedb-Distribution/**/*' ], {force: true}, cb);
});

gulp.task( 'clean', function(cb) {
	return del([ '../maxfootedb_dist/Maxfootedb-Distribution/**/*', '!../maxfootedb_dist/Maxfootedb-Distribution/maxfootedb.zip' ], {force: true}, cb);
});
