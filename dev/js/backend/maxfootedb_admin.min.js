...
jQuery(document).ready(function ($) {
	...
	// Function to add search parameters to the address bar
	maxfootedbAdminSearchDatabase();

	// Function to toggle all vendors
	maxfooteDbToggleAllVendors();

	// Function to set search fields
	maxfooteDbSetSearchFields();

	// Function to reset search fields
	maxfooteDbResetSearchFields();

	// Function for enabling the 'Upload Files' buttons.
	maxfootDBWordPressFileUpload();


	// Function for enabling the 'Upload Files' buttons.
	function maxfootDBWordPressFileUpload() {
		...
		$(document).on('click', '.maxfoote-form-section-fields-input-file-upload-button', function (event) {
		...
		$( document ).on( 'click', '#maxfoote-form-bulkupload-button-actual', function( event ) {

			var finalDocUrl = $( '#maxfoote-form-bulkupload' ).val();
			console.log("admin.min.js");
			console.log(finalDocUrl)


			var data = {
				'action': 'maxfootedb_admin_bulk_csv_upload_action',
				'security': maxfooteDbPhpVariables.adminnonce5,
	...
	// This is a function to reset all search field entries and reset the URL
	function maxfooteDbResetSearchFields() {

		$(document).on('click', '#maxfoote-reset-search-fields', function (event) {
			// var city_search_field = $('#maxfootedb-search-cities');
			// var state_search_field = $('#maxfootedb-search-states');
			// var zip_search_field = $('#maxfootedb-search-zips');
			// var trade_search_field = $('#maxfootedb-search-trades');
			// var certs_search_field = $('#maxfootedb-search-certs');

			// city_search_field.val(null);
			// state_search_field.val(null);
			// zip_search_field.val(null);
			// trade_search_field.val(null);
			// certs_search_field.val(null);

			var currenturl = window.location.href;
			var s = currenturl.substring(0, currenturl.indexOf('&city'));
			// console.log(s);
	...
	// This is a function to set the selected option in search fields
	function set_search_field(url_param, select_field) {
		if ("null" != url_param) {
			select_field.val(url_param);
		};

		return;
	};

	// This is a function to set all search fields = values in url
	function maxfooteDbSetSearchFields() {
		$(document).ready(function () {
			var currenturl = window.location.href;

			var url = new URL(currenturl);
			var city_search = url.searchParams.get("city");
			var state_search = url.searchParams.get("state");
			var zip_search = url.searchParams.get("zip");
			var trade_search = url.searchParams.get("trade");
			var certs_search = url.searchParams.get("certs");
			...
			set_search_field(city_search, city_search_field);
			set_search_field(state_search, state_search_field);
			set_search_field(zip_search, zip_search_field);
			set_search_field(trade_search, trade_search_field);
			set_search_field(certs_search, certs_search_field);
		});
	};

	// This is a function to set accordion functionality 
	...
	// This is a function to check if an entry is empty
	...
	// This is a function to toggle between all vendors and the search results
	function maxfooteDbToggleAllVendors() {
		$(document).on('click', '#maxfoote-toggle-all-vendors', function (event) {
			var all_vendors = document.getElementsByClassName("maxfoote-all-vendors");

			for (var i = 0; i < all_vendors.length; i++) {
				if (all_vendors[i].style.display === "block") {
					all_vendors[i].style.display = "none";
				} else {
					all_vendors[i].style.display = "block";
				}
			};
	...
	function maxfooteDbAdminDeleteEntry() {
		$(document).on('click', '.maxfoote-delete-vendor', function (event) {
			...
			if (answer) {
				...
				var vendorcity = $('#maxfoote-form-admin-vendorcity'+id).val();
				var vendorstate = $('#maxfoote-form-admin-vendorstate'+id).val();
				var vendorzip = $('#maxfoote-form-admin-vendorzip'+id).val();
				var vendortrade = $('#maxfoote-form-admin-vendortrade'+id).val();
				var vendorcerts = $('#maxfoote-form-admin-vendorcerts'+id).val();
				// console.log(id);
				var data = {
	...
	// Function to add search parameters to the address bar
	function maxfootedbAdminSearchDatabase() {
		$(document).on('click', '#maxfootedb-search-button', function (event) {

			var vendorcity = $('#maxfootedb-search-cities').val();
			var vendorstate = $('#maxfootedb-search-states').val();
			var vendorzips = $('#maxfootedb-search-zips').val();
			var vendortrades = $('#maxfootedb-search-trades').val();
			var vendorcerts = $('#maxfootedb-search-certs').val();
			var currenturl = window.location.href;

			var s = currenturl.substring(0, currenturl.indexOf('&tab=vendors2'));

			s = '&city=' + vendorcity + '&state=' + vendorstate + '&zip=' + vendorzips + '&trade=' + vendortrades + '&certs=' + vendorcerts;

			// console.log(currenturl + s);

			window.location = currenturl + s;
	...
	function maxfooteDbAdminSaveToDatabase() {
		$(document).on('click', '#maxfoote-admin-save-vendor', function (event) {
			...
			var vendorlastupdated = todays_date;
			var eventlocation = $('#maxfoote-form-newsite-eventlocation').val();
			var vendornotes = $('#maxfoote-form-newsite-vendornotes').val();
	...
	function maxfooteDbAdminUpdateToDatabase() {
		$(document).on('click', '.maxfoote-update-vendor', function (event) {
			var id = $(this).attr('data-dbid');
			var query_type = $(this).attr('query-type');
			...
			if ('all-vendors' === query_type) {
				...
				var vendorcontact = $('#vendorcontact' + id).val();
				var vendoremail = $('#vendoremail' + id).val();
				var vendorenterprise = $('#vendorenterprise' + id).val();
				var vendorlastupdated = todays_date;
				var eventlocation = $('#eventlocation' + id).val();
				var vendornotes = $('#vendornotes' + id).val();

			} else if ('search' === query_type) {

				var vendorname = $('#search_vendorname' + id).val();
				var vendortype = $('#search_vendortype' + id).val();
				var vendorcerts = $('#search_vendorcerts' + id).val();
				var vendorlicense = $('#search_vendorlicense' + id).val();
				var vendortrade = $('#search_vendortrade' + id).val();
				var vendoraddress = $('#search_vendoraddress' + id).val();
				var vendoraddress2 = $('#search_vendoraddress2' + id).val();
				var vendorcity = $('#search_vendorcity' + id).val();
				var vendorstate = $('#search_vendorstate' + id).val();
				var vendorzip = $('#search_vendorzip' + id).val();
				var vendorphone = $('#search_vendorphone' + id).val();
				var vendorcontact = $('#search_vendorcontact' + id).val();
				var vendoremail = $('#search_vendoremail' + id).val();
				var vendorenterprise = $('#search_vendorenterprise' + id).val();
				var vendorlastupdated = todays_date;
				var eventlocation = $('#search_eventlocation' + id).val();
				var vendornotes = $('#search_vendornotes' + id).val();

			} else {
				wp_die("maxfootedb_admin.min.js - Something is very wrong");
			}
