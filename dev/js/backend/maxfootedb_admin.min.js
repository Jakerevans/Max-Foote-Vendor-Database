...
jQuery(document).ready(function ($) {
	...
	function maxfootedbAdminSearchDatabase(){
		$(document).on('click', '#maxfootedb-search-button', function (event) {
			...
			var vendorstate = $('#searchstate').val();
...
/*
			var data = {
				'action': 'maxfootedb_admin_search_db_action',
				'security': maxfooteDbPhpVariables.adminnonce5
			};

			if(!isEmpty(vendorcity)){
				array_push(data,{
					'vendorcity': vendorcity
				})
			}

			if(!isEmpty(vendorstate)){
				array_push(data,{
			...
			if(!isEmpty(vendorzip)){
				array_push(data,{
					'vendorzip': vendorzip
				})
			}

			if(!isEmpty(vendortrades)){
				array_push(data,{
					'vendortrades': vendortrades
				})
			}

			if(!isEmpty(vendorcerts)){
				array_push(data,{
			...
			$.post(ajaxurl, data, function (response) {
				console.log('Please search the db');				
			});
*/
