...
if ( ! class_exists( 'Maxfoote_Admin_Menu', false ) ) :
/**
 * Maxfoote_Admin_Menu Class.
 */
class Maxfoote_Admin_Menu {
    ...
    public function __construct() {
        ...
        switch ( $this->page ) {
            ...
            case 'MaxFootedb-Options-submenu-page1':
                ...
                break;  
             case 'MaxFootedb-Options-submenu-page2':
    ...
    private function setup_settings_ui() {
        ...
        if(has_filter('maxfootedb_add_tab_settings')) {
            $this->tabs = apply_filters('maxfootedb_add_tab_settings', $this->tabs);
        }
    ...
    private function setup_submenupage1_ui() {
        $this->tabs = array(
            'submenupage1tab1'   => __("Submenu1 Tab 1", 'maxfoote'),
            'submenupage1tab2'  => __("Submenu1 Tab 2", 'maxfoote'),
        );

        if(has_filter('maxfootedb_add_tab_submenupage1')) {
            $this->tabs = apply_filters('maxfootedb_add_tab_submenupage1', $this->tabs);
        }
    ...
    private function setup_submenupage2_ui() {
        $this->tabs = array(
            'submenupage2tab1'   => __("Submenu2 Tab 1", 'maxfoote'),
            'submenupage2tab2'  => __("Submenu2 Tab 2", 'maxfoote'),
        );

        if(has_filter('maxfootedb_add_tab_submenupage2')) {
            $this->tabs = apply_filters('maxfootedb_add_tab_submenupage2', $this->tabs);
        }
    ...
    private function setup_dynamic_ui() {
        ...
        foreach($tab_slug_array as $key=>$slug){
            $this->tabs[$slug] = __($tab_display_array[$key], 'maxfoote');
        }

        // A filter to add tabs to the submenu page. So the submenu extensions can have their own separate plugins that add tabs to it. The name of this filter will be 'maxfootedb_add_tab_' plus the one-word unique identifer for this extension, i.e., the word that is displayed in the Maxfoote plugin main menu.  
        if(has_filter('maxfootedb_add_tab_'.$page[2])) {
            $this->tabs = apply_filters('maxfootedb_add_tab_'.$page[2], $this->tabs);
        }
    ...
    private function output_indiv_tab() {
        ...
        if(!file_exists(MAXFOOTEDB_ROOT_INCLUDES_UI_ADMIN_DIR.$filename)){
            $path = $this->build_extension_path();
            if(is_dir($path)){
           ...
           require_once(MAXFOOTEDB_ROOT_INCLUDES_UI_ADMIN_DIR.$filename);
        }
    }
    ...
    private function build_extension_path() {
        ...
        foreach($this->active_plugins as $plugin){
            if(strpos($plugin, 'maxfoote-') !== false){
                ...
                if(file_exists($path) && !is_dir($path)){
                    ...
                    if(strpos($plugin, $page[2]) !== false){
                        $path = ROOT_WP_PLUGINS_DIR.'maxfoote-'.$page[2];
                        if(file_exists($path)){
                            return $path;
                        }
                    }
